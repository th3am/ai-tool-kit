<div class="contents" x-data="registerForm()">
    ...
    <div class="flex-1 w-full max-w-md px-4 sm:px-6 md:px-0">
        <div class="relative backdrop-blur-3xl bg-white/50 dark:bg-white/10 rounded-2xl p-6 sm:p-8 md:p-10 border border-gray-200 dark:border-white/5 transition-all duration-700 ease-in-out">
            ...
            <form wire:submit="register" class="space-y-5 relative z-10">
                <div>
                    ...
                    <input type="text" wire:model="name" placeholder="John Doe" class="w-full h-[50px] bg-[#DBDDE9] dark:bg-white/10 text-gray-800 dark:text-white px-4 py-2 rounded-lg outline-none placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 transition"/>
                    @error('name') <span class="text-red-500 text-sm mt-1 block font-semibold pl-1">{{ $message }}</span> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="email" wire:model="email" placeholder="you@example.com" class="w-full h-[50px] bg-[#DBDDE9] dark:bg-white/10 text-gray-800 dark:text-white px-4 py-2 rounded-lg outline-none placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 transition"/>
                    @error('email') <span class="text-red-500 text-sm mt-1 block font-semibold pl-1">{{ $message }}</span> @enderror
                </div>
                ...
                @error('whatsapp_number') <span class="text-red-500 text-sm mt-1 block font-semibold pl-1">{{ $message }}</span> @enderror
                ...
                <div>
                    ...
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" wire:model="password" placeholder="••••••••••" class="w-full h-[50px] bg-[#DBDDE9] dark:bg-white/10 text-gray-800 dark:text-white px-4 pr-10 py-2 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition"/>
                    ...
                    @error('password') <span class="text-red-500 text-sm mt-1 block font-semibold pl-1">{{ $message }}</span> @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input :type="showPassword ? 'text' : 'password'" wire:model="password_confirmation" placeholder="••••••••••" class="w-full h-[50px] bg-[#DBDDE9] dark:bg-white/10 text-gray-800 dark:text-white px-4 pr-10 py-2 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition"/>
                    </div>
                </div>

                <button type="submit" class="w-full bg-gradient-to-r from-[#3b82f6] to-[#6366f1] hover:from-[#2563eb] hover:to-[#4f46e5] text-white py-2.5 rounded-lg font-semibold transition-all duration-200 shadow-lg disabled:opacity-50 relative">
                ...
                <p class="text-center text-gray-600 dark:text-gray-400 mt-4 text-sm">
                    ...
                    <a href="{{ route('login') }}" wire:navigate class="text-blue-500 dark:text-blue-400 hover:underline">Sign In</a>
                </p>
            </form>
        </div>
    </div>
            ...
            return {
                ...
                init() {
                    ...
                    const syncToLivewire = () => {
                        const raw = this.iti.getNumber();
                        const countryData = this.iti.getSelectedCountryData();
                        const clean = raw.replace('+', '');
                        
                        @this.set('whatsapp_number', clean);
                        @this.set('country_code', countryData.iso2);
                        @this.set('country_name', countryData.name);
                    };

                    input.addEventListener("countrychange", syncToLivewire);
                    input.addEventListener("blur", syncToLivewire);
                    input.addEventListener("input", () => {
                        // Debounce? For now just sync on blur/change to avoid too many requests
                    });
                    
                     // Initial Sync (maybe wait for auto geoip)
                    input.addEventListener("countrychange", syncToLivewire);
                }
            }
        }
